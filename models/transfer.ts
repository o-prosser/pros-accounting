...
export const selectTransfers = cache(
  async (params?: {
    ...
    const transfers = await db.query.transfersTable.findMany({
      where: (fields, { and, or, eq }) =>
        and(
          ...
          params?.account
            ? or(
                eq(fields.from, params?.account),
                eq(fields.to, params?.account),
              )
            : undefined,
...
export const selectTransfer = cache(async (params: { id: string }) => {
  const organisation = await selectCurrentOrganisation();

  const transfer = await db.query.transfersTable.findFirst({
    where: (fields, { and, eq }) =>
      and(eq(fields.organisationId, organisation.id), eq(fields.id, params.id)),
  });
